...
const TARGET_URL = "https://app-chiltonlibrary-com.oh0117.oplin.org/home";
...
selectBtn.addEventListener("click", () => {
  ...
  if (!year || !make || !model) {
    vehicleMessage.textContent = "Complete Year, Make and Model to continue.";
    vehicleMessage.classList.add("error");
    return;
  }

  const summary = `${year} ${make} ${model}`;
  vehicleMessage.textContent = `Vehicle selected: ${summary}`;
  vehicleMessage.classList.add("success");

  try {
    localStorage.setItem("chilton_vehicle", summary);
  } catch (error) {
...
resetBtn.addEventListener("click", () => {
  ...
  vehicleMessage.textContent = "Form reset successfully.";
  vehicleMessage.className = "vehicle-message success";
});

recentSelect.addEventListener("change", () => {
  if (recentSelect.value === "Last Search" || recentSelect.value === "Saved Vehicle") {
    const lastVehicle = localStorage.getItem("chilton_vehicle");

    vehicleMessage.className = "vehicle-message";

    if (lastVehicle) {
      vehicleMessage.textContent = `Last saved vehicle: ${lastVehicle}`;
      vehicleMessage.classList.add("success");
    } else {
      vehicleMessage.textContent = "No saved vehicle found.";
      vehicleMessage.classList.add("error");
    }
  }
});
...
accessForm.addEventListener("submit", (event) => {
  ...
  if (enteredCode === VALID_CODE) {
    accessMessage.textContent = "Access granted. Opening portal...";
